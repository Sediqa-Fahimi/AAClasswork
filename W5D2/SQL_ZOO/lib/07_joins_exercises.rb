...
def travoltas_busiest_years
  ...
  execute(<<-SQL)
    select yr,count(title) as totals
    from movies
    join castings on movies.id = castings.movie_id 
    join actors on castings.actor_id = actors.id AND actors.name = 'John Travolta'
    group by yr
...
def prolific_actors
  ...
  execute(<<-SQL)
   select actors.name
   from actors
   join castings on actors.id = castings.actor_id AND ord = 1
   group by name
   having count(ord) >= 15
   order by name asc
  SQL
